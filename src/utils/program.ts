  ...
  workouts: Array.from({ length: 4 }).flatMap((_, week) => [
    {
      ...
      exercises: [
        ...
        { id: "33", sets: 3, repRange: [12, 15], isAmrap: week === 3 }, // AMRAP only on last week
      ],
    },
    {
      dayIndex: week * 7 + 2,
...
export const strength_5day: Program = {
  ...
  workouts: Array.from({ length: 6 }).flatMap((_, week) => {
    const intensity = week < 2 ? 0 : week < 4 ? 1 : 2;
    const reps = [
      [8, 10],
      [5, 8],
      [3, 5],
    ][intensity] as [number, number];
    ...
    return [
      ...
      },
      {
        ...
        exercises: [
          ...
          { id: "17", sets: 3, repRange: [8, 12], isAmrap: week === 5 }, // Peak Week AMRAP
...
export const full_body_2day: Program = {
  ...
  workouts: Array.from({ length: 12 }).flatMap((_, week) => {
    const cycle = Math.floor(week / 4); // Changes every 4 weeks
    return [
      {
        dayIndex: week * 7 + 0,
        label: "Full Body A",
...
// Make sure to add it to your exported list
