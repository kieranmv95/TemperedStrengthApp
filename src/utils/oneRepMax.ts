 ...
 */
export function calculateOneRepMaxKg(
  weightKg: number,
  reps: number
): number | null {
...
export function formatDisplayWeight(weight: number, unit: WeightUnit): string {
  ...
  const formatted = unit === 'lb' ? String(Math.round(weight)) : String(weight);
  return `${formatted} ${suffix}`;
}

export function parseLiftWeightToKg(
  raw: string,
  unit: WeightUnit
): number | null {
...
export function oneRepMaxKgToDisplay(
  oneRepMaxKg: number,
  unit: WeightUnit
): number {
